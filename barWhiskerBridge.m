...
bridgeGap = 0.050*max(inBar(:)+inWhisker(:));
...
for iterCluster = 1:enn
    ...
    for iterBar = 1:emm
        if ~isnan(inBar(iterCluster,iterBar)) && ~isnan(inWhisker(iterCluster,iterBar))
            ...
            line( (XmiddleNow*ones(2,1)) , [Ybar;YwhiskerNow] ,'color','k','linewidth',2)
    ...
    for iterOne = 2:emm
        for iterTwo = 1:(emm-iterOne+1)
            ...
            if inBridge(num1,num2,iterCluster) ~= 0
                ...
                %now draw the bridge
                line( [Xl ; Xr] , Ytop*ones(2,1) ,'color','k','linewidth',1)
                line( Xl*ones(2,1) , [Ywhiskers(numL);Ytop] ,'color','k','linewidth',1)
                line( Xr*ones(2,1) , [Ywhiskers(numR);Ytop] ,'color','k','linewidth',1)
